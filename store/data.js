...
import { savePlan, getPlanList, updateIndex, rebuildIndex, exportJson, exportCsv, savePlanTemplate, convertSubtasksToChildPlans } from '@/utils/storage.js'
...
export const useDataStore = defineStore('data', () => {
  ...
  const { execCreatePlan, execUpdatePlan, execUpdatePlanPhase, execUpdatePlanSubtask, execDeletePlan, execQueryPlan, execCreatePlanTemplate } = createPlanExecutors(ctx)
  ...
  function createPlanFromTemplate(tpl) {
    ...
    const plan = {
      ...
      priority: Math.min(2, Math.max(0, tpl.plan_data?.priority ?? 2)),
      status: 1,
    ...
    // 模板子任务 → 子计划（新模型：计划只包含子计划）
    const specs = convertSubtasksToChildPlans(tpl.plan_data?.subtasks, { priority: plan.priority })
