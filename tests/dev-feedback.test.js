...
describe('autoSelect', () => {
  it('getMessageIssue 识别失败/异常消息', () => {
    expect(getMessageIssue({ role: 'assistant', content: '请求失败: Access denied', failed: true })).toEqual({ level: 2, reason: 'failed' })
    expect(getMessageIssue({ role: 'assistant', content: 'AI 走神了，要不要再试一次？' })).not.toBeNull()
    expect(getMessageIssue({ role: 'assistant', content: '好的，已记录' })).toBeNull()
    expect(getMessageIssue({ role: 'user', content: '帮我记账' })).toBeNull()
  })

  it('getMessageIssue 识别残缺的 [执行结果: 回声', () => {
    expect(getMessageIssue({ role: 'assistant', content: '好的\n[\u6267\u884c\u7ed3\u679c: \u8ba1\u5212\u6536\u5230' })).not.toBeNull()
  })

  it('selectIssueConversations 按问题消息数排序', () => {
    const convs = [
      { id: 'a', updatedAt: 1, messages: [{ role: 'user', content: '你好' }] },
      { id: 'b', updatedAt: 2, messages: [
        { role: 'user', content: '你好' },
        { role: 'assistant', content: 'AI 走神了' },
        { role: 'assistant', content: '请求失败' }
      ] },
      { id: 'c', updatedAt: 3, messages: [{ role: 'assistant', content: 'AI 响应超时' }] }
    ]
    const picked = selectIssueConversations(convs, { limit: 2 })
    expect(picked.map(p => p.conv.id)).toEqual(['b', 'c'])
    expect(picked[0].issueCount).toBe(2)
  })

  it('selectKeyMessages 保留异常消息前后上下文', () => {
    const conv = { id: 'x', messages: [
    ...
    const msgs = selectKeyMessages(conv, { contextBefore: 1, contextAfter: 1 })
    expect(msgs.map(m => m.content)).toEqual(['m3', '请求失败', 'm5'])
  })

  it('selectKeyMessages 无异常时返回最近 maxMessages 条', () => {
    const msgs = []
    for (let i = 0; i < 10; i++) msgs.push({ role: 'user', content: 'm' + i })
    const out = selectKeyMessages({ messages: msgs }, { maxMessages: 4 })
