...
describe('extractFallbackAction — 关系图谱兜底', () => {
  it('用户说"汪澄是我女朋友" + AI回复"已记录" → 提取 create_relation', () => {
    ...
    expect(action.payload.role).toBe('女朋友')
  })
  ...
  it('用户说"张三是我同事" → relation=同事', () => {
    ...
    expect(action.payload.role).toBe('同事')
  })
  ...
  it('用户说"李四是我老婆" → relation=老婆', () => {
    ...
    expect(action.payload.role).toBe('老婆')
  })
  ...
  it('用户说"叫王五，是我同学" → name=王五 relation=同学', () => {
    ...
    expect(action.payload.role).toBe('同学')
  })
  ...
  it('用户说"认识了一个新朋友叫阿伟，产品经理" → create_relation（role=朋友，context=产品经理）', () => {
    const action = extractFallbackAction('认识了一个新朋友叫阿伟，产品经理', '好的，已帮你记住阿伟啦')
    expect(action).not.toBeNull()
    expect(action.type).toBe('create_relation')
    expect(action.payload.name).toBe('阿伟')
    expect(action.payload.role).toBe('朋友')
