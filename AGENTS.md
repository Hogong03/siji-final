...
| 代码量 | ~196 文件 / ~34,000 行 |
| 测试 | 74 文件 / 1061 用例，Vitest，`npx vitest run --maxWorkers=2` 实测全绿（exit 0，无日期相关失败用例） |
| 版本 | v3.10.0（开场对话整合 + 按钮全部可点（修 `$root.$emit` 与 `uni.$on` 不通的死按钮）；计划时间可改（编辑值优先 + picker 换 view）且到点默认提醒（含安卓 13+ 通知权限）；文件类型放宽、图片走识别通道） |
...
- 测试必须带资源限制跑：$env:NODE_OPTIONS="--max-old-space-size=4096"; npx vitest run --maxWorkers=2 —— 直接 `npx vitest run` 会 OOM（op-claim-guard 测试也依赖它）；实测 74 文件 / 1061 用例全绿（exit 0）
