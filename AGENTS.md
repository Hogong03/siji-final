...
| 代码量 | ~196 文件 / ~34,000 行 |
| 测试 | 69 文件 / 985 用例，Vitest，`npx vitest run --maxWorkers=2` 实测全绿（exit 0，无日期相关失败用例） |
| 版本 | v3.7.4（自检报告自带口径版本与应用版本 —— 跑的是哪套代码一眼能认；语料条数笔误修正为 23） |
...
- 测试必须带资源限制跑：$env:NODE_OPTIONS="--max-old-space-size=4096"; npx vitest run --maxWorkers=2 —— 直接 `npx vitest run` 会 OOM（op-claim-guard 测试也依赖它）；实测 69 文件 / 985 用例全绿（exit 0）
