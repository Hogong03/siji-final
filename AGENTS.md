...
| 代码量 | ~196 文件 / ~34,000 行 |
| 测试 | 70 文件 / 1007 用例，Vitest，`npx vitest run --maxWorkers=2` 实测全绿（exit 0，无日期相关失败用例） |
| 版本 | v3.7.7（Android 选文件真因：Uri 被 `String()` 字符串化导致 `openInputStream` 失败 —— 必须传 Java 对象；开流三级 `openInputStream` → `openFileDescriptor` → `openAssetFileDescriptor`，阶段名拆细） |
...
- 测试必须带资源限制跑：$env:NODE_OPTIONS="--max-old-space-size=4096"; npx vitest run --maxWorkers=2 —— 直接 `npx vitest run` 会 OOM（op-claim-guard 测试也依赖它）；实测 70 文件 / 1007 用例全绿（exit 0）
