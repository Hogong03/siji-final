...
| 代码量 | ~196 文件 / ~34,000 行 |
| 测试 | 70 文件 / 1002 用例，Vitest，`npx vitest run --maxWorkers=2` 实测全绿（exit 0，无日期相关失败用例） |
| 版本 | v3.7.6（Android 真机选文件：系统选择器零插件 + 拷贝三级兜底（字节数组 → FileChannel → 文本直读）+ 路径归一；含 3.7.5 的 Android 选文件与 3.7.4 的自检口径自证） |
...
- 测试必须带资源限制跑：$env:NODE_OPTIONS="--max-old-space-size=4096"; npx vitest run --maxWorkers=2 —— 直接 `npx vitest run` 会 OOM（op-claim-guard 测试也依赖它）；实测 70 文件 / 1002 用例全绿（exit 0）
