...
- **结合用户数据**：如果用户开启了个人信息，可以结合日记心情数据发现规律（"我注意到你这周三和周四的日记里都提到了'压力'，有什么共同点吗？"）
...
## 技术能力
你仍然具备思迹的全部核心能力（记账/日记/计划/个人信息），可以在心理咨询过程中自然地结合这些数据进行洞察，但不要主动切换到"功能操作模式"。`
  },
  {
    name: '健身教练',
    avatar: '🏋️',
...
## 技术能力
你仍然具备思迹的全部核心能力（记账/日记/计划/个人信息），可以帮用户创建训练计划、记录健身日志、分析饮食消费。`
  },
  {
    name: '财务顾问',
    avatar: '💰',
...
## 技术能力
你仍然具备思迹的全部核心能力（记账/日记/计划/个人信息），可以帮用户记账、查看消费趋势、设置预算提醒、制定储蓄计划。`
  },
  {
    name: '学习伙伴',
    avatar: '📚',
...
## 技术能力
你仍然具备思迹的全部核心能力（记账/日记/计划/个人信息），可以帮用户创建学习计划、记录学习日记、追踪每日投入时间。`
  },
  {
    name: '极简助手',
    avatar: '✨',
...
## 技术能力
你仍然具备思迹的全部核心能力（记账/日记/计划/个人信息），但所有操作的确认都以最短格式呈现。`
  }
]
  ...
  <view class="agent-add-page">
      ...
      <view class="edit-form">
        ...
        <view class="form-section">
          ...
          <view class="avatar-grid">
            <view
              v-for="emoji in AVATAR_OPTIONS" :key="emoji"
              class="avatar-option"
              :class="{ active: editForm.avatar === emoji }"
              @tap="editForm.avatar = emoji"
            >
              <text>{{ emoji }}</text>
            </view>
          </view>
    ...
    <view class="form-actions-bar">
      <view class="form-actions">
        <button class="btn-cancel" @tap="uni.navigateBack()">取消</button>
...
.form-actions-bar {
  ...
  padding-bottom: calc(#{$spacing-sm} + env(safe-area-inset-bottom, 0rpx));
  background: var(--bg-page, #FFFFFF);
