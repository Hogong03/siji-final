      ...
      <view class="rank-section">
        ...
        <view v-for="r in sortedRelations" :key="r.id" class="rank-row" @tap="goDetail(r.id)">
          <view class="rank-left">
            <view class="rank-avatar" :class="{ 'rank-self': r.is_builtin }">{{ r.name.charAt(0) }}</view>
...
const selfName = computed(() => basicCard.value.fields.nickname || '庚哥')
const selfBio = computed(() => basicCard.value.fields.bio || '追求意义与真实，忠于内心选择')
...
const selfTraits = computed(() => {
  const gengge = relations.value.find(r => r.id === 'builtin_gengge')
  return gengge?.traits || ['理想主义', '内敛', '共情力强', '珍视内心真实', '感性']
})
...
const graphNodes = computed(() => {
  return relations.value.map(r => {
    ...
    const isSelf = r.is_builtin === 1 || r.id === 'builtin_gengge'
    return {
      id: r.id,
      name: r.name,
      symbolSize: isSelf ? 60 : (20 + score * 4), // 自己最大
      category: isSelf ? 0 : (score >= 8 ? 1 : (score >= 5 ? 2 : 3)),
      value: score,
      itemStyle: {
        color: isSelf ? '#000000' : (score >= 8 ? '#18181B' : (score >= 5 ? '#71717A' : '#D4D4D8')),
        borderColor: '#FFFFFF',
        borderWidth: 2
      },
      label: {
...
const graphLinks = computed(() => {
  // 自己连接所有人物
  const links = []
  const self = relations.value.find(r => r.is_builtin === 1 || r.id === 'builtin_gengge')
  const selfId = self ? self.id : 'builtin_gengge'
